...
# Deep stealth patches appended to every context's init script. Loaded from
# ``assets/js/hardening.js`` so the JS is lintable and versioned independently
# of the Python f-string head. Placeholder order (all consumed once):
...
def _hardening_js_template() -> str:
    return load_js("hardening.js")
